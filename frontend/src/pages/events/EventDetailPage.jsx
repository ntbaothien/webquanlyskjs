...
function ShareButtons({ event, eventId }) {
  ...
  const displayTime = new Date(event.startDate).toLocaleString(locale, { dateStyle: 'full', timeStyle: 'short' });
  const richText = `🔥 Tham gia ngay sự kiện: ${event.title.toUpperCase()} 🔥\n\n📍 Địa điểm: ${event.location || 'Trực tuyến'}\n⏰ Thời gian: ${displayTime}\n\n👉 Nhanh tay đặt chỗ tại đây: `;

  const handleShare = (platform) => {
    const p = SHARE_PLATFORMS.find(x => x.key === platform);
    if (!p) return;
    const shareUrl = platform === 'facebook' ? ogUrl : url;
    window.open(p.getUrl(shareUrl, richText), '_blank', 'width=640,height=480,noopener,noreferrer');
    setShareCount(c => c + 1);
  };

  const handleNativeShare = async () => {
    try {
      await navigator.share({ title: event.title, text: richText, url });
      setShareCount(c => c + 1);
    } catch {}
  };
