...
export default function CheckoutPage() {
  ...
  const validateDiscount = async () => {
    ...
    try {
      ...
      setDiscountError(err.response?.data?.error || t('checkout.invalidCode'));
      setDiscount(null);
    }
  };
  ...
  return (
    <div className="page">
      <div className="container page-content" style={{ maxWidth: 800 }}>
        <h2 className="mb-8">{t('checkout.title')}</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 24 }}>
          ...
          <div>
            <div className="card mb-20">
              <h3 className="mb-16">{t('checkout.selectTicket')}</h3>
              {ticketTypes.map(tt => (
                <div key={tt._id}
                  ...
                  <div className="flex-between">
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: 'var(--text-dim)', marginTop: 4 }}>{t('checkout.remaining', { count: tt.quantity - tt.sold })}</div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontWeight: 700, color: 'var(--primary-light)', fontSize: 16 }}>
                        {tt.price === 0 ? t('checkout.free') : `${tt.price.toLocaleString(locale)}đ`}
                      </div>
            ...
            {selected && (
              <div className="card mb-20">
                <h3 className="mb-16">{t('checkout.quantity')}</h3>
                <div className="flex" style={{ alignItems: 'center', gap: 16 }}>
                  ...
                  <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>/ {t('checkout.remaining', { count: selected.quantity - selected.sold })}</span>
                </div>
              </div>
            )}

            <div className="card">
              <h3 className="mb-16">{t('checkout.discountCode')}</h3>
              <div className="flex gap-12">
                <input className="form-input" placeholder={t('checkout.discountPlaceholder')}
                  value={discountCode} onChange={e => { setDiscountCode(e.target.value.toUpperCase()); setDiscount(null); }} />
                <button className="btn btn-secondary" onClick={validateDiscount}>{t('checkout.apply')}</button>
              </div>
              {discountError && <div className="form-error mt-8">{discountError}</div>}
              {discount && (
                <div className="alert alert-success mt-12">
                  {t('checkout.discountApplied', { amount: discount.discountAmount.toLocaleString(locale), code: discount.code })}
                </div>
              )}
            </div>
          </div>
          ...
          <div>
            <div className="card-glass" style={{ padding: 24, position: 'sticky', top: 'calc(var(--header-h) + 20px)' }}>
              <h3 className="mb-16">{t('checkout.summary')}</h3>
              {selected ? (
                <>
                  <div style={{ fontSize: 14, borderBottom: '1px solid var(--border)', paddingBottom: 16, marginBottom: 16 }}>
                    <div className="flex-between mb-8"><span style={{ color: 'var(--text-muted)' }}>{selected.name}</span><span>{selected.price.toLocaleString(locale)}đ</span></div>
                    <div className="flex-between mb-8"><span style={{ color: 'var(--text-muted)' }}>{t('checkout.qty')}</span><span>×{qty}</span></div>
                    <div className="flex-between"><span style={{ color: 'var(--text-muted)' }}>{t('checkout.subtotal')}</span><span>{total.toLocaleString(locale)}đ</span></div>
                    {discount && <div className="flex-between mt-8" style={{ color: 'var(--success)' }}><span>{t('checkout.discount')}</span><span>−{discount.discountAmount.toLocaleString(locale)}đ</span></div>}
                  </div>
                  <div className="flex-between" style={{ fontWeight: 800, fontSize: 18, marginBottom: 20 }}>
                    <span>{t('checkout.total')}</span>
                    <span style={{ color: 'var(--primary-light)' }}>{finalAmount.toLocaleString(locale)}đ</span>
                  </div>
                  <button className="btn btn-primary btn-lg w-full" onClick={handleOrder} disabled={loading}>
                    {loading ? <span className="spinner spinner-sm" /> : t('checkout.confirm')}
                  </button>
